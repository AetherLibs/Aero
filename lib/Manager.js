...
module.exports = class Manager {
	...
	init() {
		...
		this.ws.on('open', () => {
			this.client.client.console.log('[Sharder] WS opened.');
			this.reconnector.handleOpen();
		});

		this.ws.on('close', (code, reason) => {
			this.client.client.console.warn('[Sharder] WS closed.');
			this.reconnector.handleClose(code, reason);
		});

		this.ws.on('error', error => {
			this.client.client.console.error('[Sharder] WS errored.');
	...
	listen() {
		...
		this.api.get('/health', (req, res) => {
			this.client.client.console.log('[Kubernetes] Sent healthy.');
			res.status(200).end();
		});

		this.api.get('/ready', (req, res) => {
			if (this.client.ready) {
				this.client.client.console.log('[Kubernetes] Sent ready.');
				res.status(200).end();
			} else {
				this.client.client.console.warn('[Kubernetes] Sent unready.');
				res.status(400).end();
			}
		});
	...
	launch(data) {
		this.client.client.console.log('[Sharder] Attempting to login.');
