...
const { reactions, emojis: { yes, no, fallbackYes, fallbackNo } } = require('../util/constants');
const emojis = [reactions.yes, reactions.no, fallbackYes, reactions.fallbackNo];
		...
		async createRaidMessage() {
			...
			message
				...
				.then(collected => {
					...
					collected.first().emoji.id === reactions.fallbackYes
