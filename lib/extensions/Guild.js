...
Structures.extend('Guild', Guild => {
	class AeroGuild extends Guild {
		...
		async createRaidMessage() {
			...
			const message = await channel.send(this.language.get('EVENT_RAID_TITLE', yes, no), { embed });
		...
		async updateRaidMessage() {
			...
			return message.edit(this.language.get('EVENT_RAID_TITLE', yes, no), await this.constructMessage());
		}
