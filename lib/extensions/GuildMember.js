...
Structures.extend('GuildMember', GuildMember => {
	class AeroMember extends GuildMember {
		...
		customAvatarURL({ size = '2048', format = 'webp' } = {}) {
			return this.customAvatar && `https://cdn.discordapp.com/guilds/${this.guild.id}/users/${this.id}/avatars/${this.customAvatar}.${format}?size=${size}`;
		}
		...
		set communicationDisabledUntil(communication_disabled_until) {
			if (!communication_disabled_until) {
				this.chatMutedTimestamp = null;
			} else {
				this.chatMutedTimestamp = new Date(communication_disabled_until).getTime();
			}
		}
